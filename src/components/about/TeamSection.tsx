...
// Allow optional 'name' for team members
type ContentItem = {
  type: 'text' | 'image';
  data: string;
  name?: string;
    ...
    <section className="flex items-center justify-center flex-col p-10 bg-black text-white relative">
      ...
      <div className="flex flex-col md:flex-row md:space-x-24">
        <div className="py-5 md:py-0">
          {girl && (
            <Image src={girl} alt="team" width={100} height={100} />
          )}
          <h5 className="text-[#3ae51d] font-bold">{teamData?.[1]?.name}</h5>
          <h6 className="text-[9px]">{teamData?.[2]?.data}</h6>
        </div>

        <div>
          {boy && (
            <Image src={boy} alt="team" width={100} height={100} />
          )}
          <h5 className="text-[#3ae51d] font-bold">{teamData?.[3]?.name}</h5>
        ...
        <div className="pt-5 md:pt-0">
          ...
          <h5 className="text-[#3ae51d] font-bold">{teamData?.[5]?.name}</h5>
